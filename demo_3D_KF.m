...
if epoch == st.start_fr % first scan
    ...
    [ gate ]                         = gate_properties ( obj );
    
    % initialize KF MS 3D
    klm.location    = klmi([ first_location(1) 0 0 first_location(2) 0 0 first_location(3) 0 0]', obj.class );
    klm.location.z  = [ first_location(1) first_location(2) first_location(3) ]';
    klm.location    = kalmanf_simplified ( klm.location );
    klm.location.next = klm.location.A * klm.location.x;
    klm.location.next  = [klm.location.next(1) klm.location.next(4) klm.location.next(7) ]';
    % obj.cent_3d = [ klm.location.x(1) klm.location.x(4) klm.location.x(7) ];
    ...
    %% gating to find centroid
    tic
    [ obj, gate ] = fun_gate_area ( klm, obj, gate, pcd );                  % define search area for 2d/3d centroid projection
    runtime.gate(vector_idx, :) = toc;
    gate_cent = obj.cent_3d;
    
    %% apply location KF
    klm.location.z  = [gate_cent(1) gate_cent(2) gate_cent(3) ]';
    klm.location    = kalmanf_simplified ( klm.location );
    klm.location.next = klm.location.A * klm.location.x;
    klm.location.next  = [klm.location.next(1) klm.location.next(4) klm.location.next(7) ]';
...
%     obj_angle_world = radtodeg(abs(atan(numerator / denominator)));
    ...
    % code: rotation fix for slow/stopped objects (checking displacements)
    n_prev_loc      = abs(diff(sqrt(sum(obj.list.cent_3d(max(1, vector_idx - n_prev) : vector_idx-1 , :).^2, 2))));
    n_prev_rot      = abs(diff(obj.list.pose_2d(max(1, vector_idx - n_prev) : vector_idx-1 )));
    if (vector_idx > n_prev + 1)
        if (n_prev_loc(end) < n_prev_loc_tr) && (n_prev_rot(end) > n_prev_rot_tr)
            obj.ry                  = obj.list.pose_2d(vector_idx - 1);
            obj.list.pose_2d(vector_idx, :) = obj.ry;
            pose                    = obj.list.pose_3d(:, :, vector_idx - 1);
            pose                    = reshape(pose(:), 3, 3);
            obj.list.pose_3d(:, :, vector_idx) = pose;
        end
    end
    
    obj.shift_3d = obj.cent_3d - orig_cent_3d;
    obj.bb_3d    = bb_calc ( obj, obj.cent_3d, 'src' );
...
% 
% % whole point cloud
% % plt.all_pcd = scatter3(pcd.world.all_pts(:, 1), pcd.world.all_pts(:, 2), pcd.world.all_pts(:, 3)+st.z_base, 1, 'r','filled');
% plt.ins_pcd = scatter3(pcd.world.in_img(:, 1), pcd.world.in_img(:, 2), pcd.world.in_img(:, 3)+st.z_base, 1, 'b','filled');
% plt.obs_pcd = scatter3(pcd.world.obs_pts(:, 1), pcd.world.obs_pts(:, 2), pcd.world.obs_pts(:, 3)+st.z_base, 1, 'w','filled');
% % object points
...
%     if obj.class == 1
% %         plot3(klm.location.next(1), klm.location.next(2), klm.location.next(3)+st.z_base, '.w', 'LineWidth', 1, 'MarkerSize', 15);%, 'Color', [0.75 0.75 0.75]);
% %         plot3(klm.location.next(1), klm.location.next(2), klm.location.next(3)+st.z_base, '*w', 'LineWidth', 1, 'MarkerSize', 5);%, 'Color', [0.75 0.75 0.75]);
%     else
% %         plot3(klm.location.next(1), klm.location.next(2), klm.location.next(3)+st.z_base, '.w', 'LineWidth', 2, 'MarkerSize', 30);%, 'Color', [0.75 0.75 0.75]);
% %         plot3(klm.location.next(1), klm.location.next(2), klm.location.next(3)+st.z_base, '*w', 'LineWidth', 2, 'MarkerSize', 10);%, 'Color', [0.75 0.75 0.75]);
%     end
%     % uniting prediction to current location
% %     plot3([obj.list.cent_3d(vector_idx, 1), klm.location.next(1)], ...
% %           [obj.list.cent_3d(vector_idx, 2), klm.location.next(2)], ...
% %           [obj.list.cent_3d(vector_idx, 3), klm.location.next(3)]+st.z_base, '-w', 'LineWidth', 1); %, 'Color', [0.5 0.5 0.5]);
%     % object speed over object
%     velocity_str = [sprintf('%.1f', obj.velocity.kph), ' km/h'];
%     t = text(obj.cent_3d(1), obj.cent_3d(2)+3, obj.cent_3d(3)+1+st.z_base, velocity_str, 'Color', 'k', 'HorizontalAlignment', 'left', 'FontSize', 10, 'FontWeight', 'bold');
...
% % view([-77, 48]);
% axis equal tight;
% % axis([(obj.cent_3d(1) - 10), (obj.cent_3d(1) + 20), ...
...
if epoch == st.end_fr
    err_3D_KF_Gate2(i) = fun_err( obj, list, runtime );
end
...
save err_3D_KF_Gate2 err_3D_KF_Gate2