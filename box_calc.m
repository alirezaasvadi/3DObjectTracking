function vt_list = box_calc(object, loc)
...
%% Box
u     = ((sqrt(dim(2)^2 + dim(3)^2))/2) * cos(-rot-pi/2 + atan(dim(2)/dim(3))); % Half of 'l' in the vehicle direction
v     = ((sqrt(dim(2)^2 + dim(3)^2))/2) * sin(-rot-pi/2 + atan(dim(2)/dim(3))); % Half of 'w' perpendicular to the vehicle direction
st    = [ loc(1) - u, loc(2) - v, loc(3)];                                      % start point
sz    = [ dim(3), dim(2),  dim(1) ];                                            % size
ps    = [ 0, 0, -rot-pi/2 ];                                                    % pose
cl    = [ 1, 0, 0 ];                                                            % color
al    = 0.2;                                                                    % alpha
vt_list = vox(st, sz, ps, cl, al);
end

function vt_list = vox(st, sz, ps, cl, al)
